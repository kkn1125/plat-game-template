...
export default class GameMapManager {
    constructor(engine) {
        ...
        Object.defineProperty(this, "gameMaps", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
    ...
    // 유닛 중심 좌표 계산
    getAxis(currentMap, controlUnit) {
    ...
    collisionBottom(currentMap, controlUnit) {
        const fields = currentMap.fields;
        /* 셀 포지션 계산 */
        const { x, y } = this.getAxis(currentMap, controlUnit);
        const unitHalfSizeX = controlUnit.size.x / 2;
        ...
        // 물 연접 부 컬리젼 범위 조정 - 물로 상단에서 접근
        {
            ...
            const field = fields?.[cellBottomY]?.[cellX];
            const fieldRight = fields?.[cellBottomY]?.[cellRightX];
            if ([fieldLeft, field, fieldRight].some((field) => field?.name === Tile.Water) &&
    ...
    collisionLeft(currentMap, controlUnit) {
        const fields = currentMap.fields;
        /* 셀 포지션 계산 */
        const { x, y } = this.getAxis(currentMap, controlUnit);
        const unitHalfSizeX = controlUnit.size.x / 2;
    ...
    collisionRight(currentMap, controlUnit) {
        const fields = currentMap.fields;
        /* 셀 포지션 계산 */
        const { x, y } = this.getAxis(currentMap, controlUnit);
        const unitHalfSizeX = controlUnit.size.x / 2;
