...
export default class Field {
  ...
  drawMap(ctx: CanvasRenderingContext2D, { worldAxisX, worldAxisY }: WorldAxis) {
  ...
  drawObject(ctx: CanvasRenderingContext2D, { worldAxisX, worldAxisY }: WorldAxis, emboss: boolean = false) {
    const { x, y } = this.position;
    const { X, Y } = GAME_CONF.MAP_CONF.DEFAULT.SIZE;
    const tile = this.name[0];
