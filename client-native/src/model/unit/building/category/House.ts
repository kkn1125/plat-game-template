...
export default class House extends Building {
  ...
  drawName(ctx: CanvasRenderingContext2D, { worldAxisX, worldAxisY }: WorldAxis) {
    ...
    ctx.strokeText(this.name.toUpperCase(), positionX + (this.size.x + 3 / 2) / 2, positionY - 220);

    /* font */
    ctx.textAlign = 'center';
    ctx.fillText(this.name.toUpperCase(), positionX + this.size.x / 2, positionY - 220);
  }
  ...
  drawCharacter(ctx: CanvasRenderingContext2D, { worldAxisX, worldAxisY }: WorldAxis, emboss: boolean = false) {
    ...
    if (emboss) {
      ...
      ctx.drawImage(
        ...
        this.size.y + overFlowX + padY * 2 + 70,
      );
    } else {
      ctx.drawImage(
        this.sprites,
        ...
        this.size.x + padX * 2 + overFlowX * 2,
        this.size.y + overFlowX + padY * 2 + 180,
      );
    }
